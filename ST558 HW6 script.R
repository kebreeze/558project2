...
whether an unknown population mean is different from a specific value. Thus, I plan to make two paramter in my function. 
we need to calculate the sample mean. Thus I used x_bar to present my samele mean. And then I need to calculate the standard
...
• Inputs should be the test statistic value, the sample size, the significance level (α), and the direction of
the alternative hypothesis (left, right, or two-sided).
...
determine_hy <- function(test_statistic, n, alpha = 0.05, side){
  ...
  if(side == "left"){
    ...
    #HA : mu < mu0 reject if t_obs < 0.05 quantile of a t-distribution with n-1 degrees of freedom
    quant <- qt( p = alpha, df = n-1)
    return(test_statistic < quant)
    
  } else if (side == "right"){
    
    #HA : mu > mu0 reject if t_obs > 0.95 quantile of a t-distribution with n-1 degrees of freedom
    quant <- qt( p = 1-alpha, df = n-1)
    return(test_statistic > quant)
    
  } else if (side == "two"){
    
    #HA : mu ̸= mu0 reject if abs(t_obs) > 0.975 quantile of a t-distribution with n-1 degrees of freedom
...
which are one side distribution and two side distribution. I used help(qt) to see how qt() works. qt(p, df, ncp, lower.tail = TRUE, log.p = FALSE), the p is vector of probabilitis, 
the df is degrees of freedom. Thus, I prepare make sure what I need to put inside of the qt(). For the probabilities, I know it is related to alpha. Normally, aplha is 0.05. 
Thus, I will set 0.05 as my defult alpha value. If the side is right, we know the probabilities equals 1 - alpha. If the side is left, we know the probabilities equals alpha.
If the side is two side, the probabilities is (1-alpha/2). Also, the degree freedom is number of obs - 1. I still use n to represent my number of obs. Thus
the df is n-1. Since it has three different condition, I consider use if else statment to show difference condition. In my return statement in the function,
I can compare t_obs(called test_stat in my function, which is my first function that used to count t_stat) with quantile of a t-distrinution(called quant in my function).
...
Test how well your function works! Use the built-in iris data set and run a few hypothesis tests. Using the
data, determine if the true mean
...
In this test, It is a two side test since the mu not equal, so my side is "two". Based on the null hypothsis, we know mu0 is 5.5. 
...
In this test, It is a one side test since the width is greater than 3.5, so my side is "right". Based on the null hypothsis, we know mu0 is 3.5. 
...
***Comments:***
In this test, It is a one side test since the width is less than 3.5, so my side is "left". Based on the null hypothsis, we know mu0 is 4. 
...
number of replications being 10000)! Then find the mean of the resulting TRUE/FALSE values as your estimatedlevel under these assumptions.
...
The next step is that I need to find how to evaluate repeatedly. I decide to use the gamma distribution function. I searched on google, I decide to use the rgamma(). To used this function, I need to know what is shape and scale parameters.
They must be positive. Based on the question, I set my shape is 0.5 & set rate as 1, and sample side is 10. Also, I know it is two sided test.
Then, I need to calculate the t statistics for this gamma distribution. After searched online, I know I need to find mean for gamma distribution, which will 
be my mu0. the mu is shape times rate. I applied my gamma function into my t test statstical function. In my return statement, I returned my  
determine whether you reject H0 function. Finally, I use mean() to find the mean.
...
I created my fun function, which just include my replicate() and mean() code. After I did those steps, I can use parLapply() to execute the above Monte Carlo study 
for combinations of sample sizes and shape values. After I get my final means, I need to print out a table giving the results in a reasonably palatable manner. 
I need to give the col name, so in my mean_longer, I gave them name. However, I need to use group_by to make it to be a wider version. 
